...
fn evaluate_literal(expression: &Literal) -> Result<Object, RuntimeError> {
    match *expression {
        ...
        Literal::Str(ref s) => Ok(Object::Str(s)),
...
pub mod env {
    ...
    pub struct Environment<'a> {
        pub scopes: Vec<HashMap<Symbol, Object<'a>>>,
        global: HashMap<Symbol, Object<'a>>,
        unique: Unique,
    }
    ...
    impl<'a> Environment<'a> {
        ...
        pub fn define(&mut self, name: Symbol, value: Object<'a>) {
        ...
        pub fn assign(
            ...
            if self.scopes[distance].contains_key(&name) {
                self.scopes[distance].insert(*name, value);
                Ok(())
            } else {
        ...
        /// searches the outer scope and then calls itselfs.
        /// Returns a runtime error if the not found
        pub fn get(&self, name: &Symbol, distance: usize) -> Result<&Object<'a>, RuntimeError> {
            if let Some(value) = self.scopes[distance].get(name) {
                return Ok(value);
            } else if let Some(value) = self.global.get(name) {
                return Ok(value);
            } else {
                Err(RuntimeError::UndefinedSymbol(*name))
            }
    ...
    #[derive(Debug, Clone)]
    pub(crate) struct EnvironmentImpl<'a> {
        outer: Option<Environment<'a>>,
    ...
    impl<'a> EnvironmentImpl<'a> {
        ...
        fn with_values(outer: &Environment<'a>) -> Self {
            EnvironmentImpl {
