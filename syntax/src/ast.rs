...
impl AstNode for Item {
    fn can_cast(kind: SyntaxKind) -> bool {
        match kind {
             | FN_DEF | TYPE_ALIAS_DEF | ENUM_DEF | CLASS_DEF | EXTERN_IMPORT_DEF => true,
            _ => false,
        }
    }
    fn cast(syntax: SyntaxNode) -> Option<Self> {
        if Self::can_cast(syntax.kind()) { Some(Item { syntax }) } else { None }
    }
    fn syntax(&self) -> &SyntaxNode { &self.syntax }
}
...
pub enum ItemKind {
    ...
    EnumDef(EnumDef),
    ClassDef(ClassDef),
    ExternImportDef(ExternImportDef),
}
impl From<FnDef> for Item {
    fn from(n: FnDef) -> Item { Item { syntax: n.syntax } }
}
impl From<TypeAliasDef> for Item {
    fn from(n: TypeAliasDef) -> Item { Item { syntax: n.syntax } }
}
impl From<EnumDef> for Item {
    fn from(n: EnumDef) -> Item { Item { syntax: n.syntax } }
}
impl From<ClassDef> for Item {
    fn from(n: ClassDef) -> Item { Item { syntax: n.syntax } }
}
impl From<ExternImportDef> for Item {
    fn from(n: ExternImportDef) -> Item { Item { syntax: n.syntax } }
}
impl Item {
    pub fn kind(&self) -> ItemKind {
        match self.syntax.kind() {
            FN_DEF => ItemKind::FnDef(FnDef::cast(self.syntax.clone()).unwrap()),
            TYPE_ALIAS_DEF => ItemKind::TypeAliasDef(TypeAliasDef::cast(self.syntax.clone()).unwrap()),
            ENUM_DEF => ItemKind::EnumDef(EnumDef::cast(self.syntax.clone()).unwrap()),
            CLASS_DEF => ItemKind::ClassDef(ClassDef::cast(self.syntax.clone()).unwrap()),
            EXTERN_IMPORT_DEF => ItemKind::ExternImportDef(ExternImportDef::cast(self.syntax.clone()).unwrap()),
            _ => unreachable!(),
        }
    }
}
...
impl SourceFile {
    pub fn items(&self) -> impl Iterator<Item = ItemList> {
        children(self)
    }
}
