...
use crate::infer::pattern_matrix::{Constructor, Pattern};
#[derive(Debug, PartialEq, Clone)]
pub struct Row(Vec<Pattern>, ExprId);
...
impl Row {
    pub fn new(pats: Vec<Pattern>, returns: ExprId) -> Self {
        Self(pats, returns)
    }
        ...
        unsafe {
            ...
            for i in 0..self.rows.len() {
                for j in 0..self.rows[i].0.len() {
                    let x: *mut Pattern = &mut self.rows[i].0[0];

                    let y: *mut Pattern = &mut self.rows[i].0[index];
                    std::ptr::swap(x, y);
                }
            }
        }
