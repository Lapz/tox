...
fn transform_expr(expr:&Expression) -> Result<ExpressionType,TypeError> {
    match *expr {
        ...
        Expression::Binary{ ref left_expr, ref right_expr,..} => {
        ...
        Expression::Literal(ref literal) => {
            ...
            match *literal {
                Float(_) => Ok(ExpressionType{exp:(),ty:Type::Float}),
                Int(_) => Ok(ExpressionType{exp:(),ty:Type::Int}),
                Str(_) => Ok(ExpressionType{exp:(),ty:Type::Str}),
                True(_) |False(_) => Ok(ExpressionType{exp:(),ty:Type::Bool}),
                Nil => Ok(ExpressionType{exp:(),ty:Type::Nil})
            }
        },
