...
ERROR_LINE_EXPECT = re.compile(r'// \[((rust|c) )?line (\d+)\] (Error.*)')
...
def rust_interpreter(name, tests):
  if name == 'rox':
    dir = 'target/debug/rox'
  else:
    dir = 'build/gen/' + name

  INTERPRETERS[name] = Interpreter('', 'rust',[dir], tests)
...
rust_interpreter('chap06_parsing', {
  ...
  def parse(self):
    ...
    for part in parts:
      ...
      if subpath in interpreter.tests:
        ...
        self.state = interpreter.tests[subpath]
    ...
    with open(self.path, 'r') as file:
      for line in file:
        ...
        if match:
          self.compile_errors.add("{1}".format(line_num, match.group(1)))
